...
export default function RegisterPage() {
  ...
  async function handleGoogleRegister() {
    ...
    // حفظ الـ role في localStorage باش نستخدمه بعد الـ callback
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: {
        data: { full_name: name, role, terms_accepted: true, cookies_accepted: true },
      },
    })
      ...
      <div className="flex-1 px-6 pt-6 pb-10">
        {step === 'role' ? (
            ...
            <div className="space-y-3">
              {ROLES.map(r => (
                <button key={r.value} onClick={() => { setRole(r.value); setStep('form') }}
                  className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-start ${role === r.value ? 'border-neutral-900 dark:border-white bg-neutral-50 dark:bg-neutral-800' : 'border-neutral-100 dark:border-neutral-800 hover:border-neutral-300'}`}>
          ...
          <>
            ...
            <p className="text-neutral-400 text-sm mb-6">حساب {ROLES.find(r => r.value === role)?.label}</p>
            ...
            <div className="space-y-3 mb-6 bg-neutral-50 dark:bg-neutral-900 rounded-2xl p-4">
              ...
              <Checkbox checked={acceptCookies} onChange={() => setAcceptCookies(!acceptCookies)}>
                أوافق على استخدام <span className="text-neutral-900 dark:text-white font-medium">ملفات تعريف الارتباط</span>
                <span className="text-red-500 ms-1">*</span>
              </Checkbox>
            </div>

            {/* Google Register */}
            <button onClick={handleGoogleRegister} disabled={googleLoading || !acceptTerms || !acceptCookies}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} className="input" placeholder="محمد أمين" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">{t('email')}</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="input" placeholder="example@email.com" required dir="ltr" />
              </div>
              <div>
            ...
            <p className="text-center text-sm text-neutral-500 mt-6">
              لديك حساب؟{' '}
              <Link href="/auth/login" className="text-neutral-900 dark:text-white font-semibold underline underline-offset-2">{t('login')}</Link>
            </p>
          </>
        )}
      </div>
